...
// pins
int motor1pin1 = 2; //IN1 pin
int motor1pin2 = 3; //IN2 pin
int motor1speed = 7; //ENA pin

/* PARAMETERS TO PID */
float Kp = 10; // Gain 
float Ki = 0.01; // Integrator  
...
float MV = 0;
float MV_manual = 0;
 float SetPoint = 30;// Define SetPoint on %
...
/* LIMITS MAX AND MIN */
int MV_MAX = 255 ;// Ouput MAX in %
int MV_MIN = 20;// Output MIN in %
int PV_max = 100; 
int PV_min = 0; 
...
void setup() {
      ...
      //on high/low one direction
      digitalWrite(motor1pin1, HIGH);
      digitalWrite(motor1pin2, LOW);

      pinMode(motor1speed, OUTPUT);;// Start port OUTPUT  
    }
...
// SET OUTPUT % TO PWM (ANALOG OUTPUT)
 ...
 {
    float PWM_out = map(mv,0,100,20,255);//Map output 0 to 255
    analogWrite(motor1speed,PWM_out); // Output PWM
  }
